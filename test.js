...
(async () => {
  ...
  if (fs.existsSync(path.join(__dirname,'users'))) {
    ...
    for (const user of users) {
      ...
      try {
        const data = await fs.readFile(path.join(__dirname,'users', user), 'utf8')
        ...
        try {
          ...
          if (parsedData.version) {
            await report(`Version tag found in ${user}`, async () => {
              ...
              await writeJsonFile(path.join(__dirname,'users', user), parsedData, { indent: 2 })
            })
          }
          ...
          if (is.string(parsedData.gravatar)) {
            await report(`Gravatar boolean encoded as string found in ${user}`, async () => {
              ...
              await fs.writeFile(path.join(__dirname,'users', user), stringified)
            })
          }
        } catch ({ message }) {
  ...
  if (fs.existsSync(path.join(__dirname,'themes'))) {
    ...
    const themes = await fs.readdir(path.join(__dirname,'themes'))

    for (const theme of themes) {
      if (getExtension(theme) === 'css') {
        try {
          const cssData = await fs.readFile(path.join(__dirname,'themes', theme), 'utf8')
          try {
            CSS.parse(cssData)
          } catch ({ message }) {
